...
import { fetchLevels, fetchDistricts, fetchUniversities, fetchTypes, fetchCoursesDropdown } from "@/utils/api"
...
export default function SchoolFilters({ filters, onChange }) {
  ...
  useEffect(() => {
    const loadData = async () => {
      setLoading(true)
      try {
        const [levelsData, districtsData, universitiesData, typesData, coursesData] = await Promise.all([
          fetchLevels(),
          fetchDistricts(),
          fetchUniversities(),
          fetchTypes(),
      ...
      <div className="md:hidden mb-4">
        ...
        >
          <div className="flex items-center gap-2">
            ...
            <span className="font-medium text-gray-700">Filter Schools</span>
          </div>
          {hasActiveFilters && (
